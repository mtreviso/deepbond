...
class ErrorAnalysisSS:
    ...
    def __init__(self, gold_dir, pred_dir):
        ...
        self.macro_report(gold_dir, pred_dir)
    ...
    def _populate(self, gold_list, pred_list):
        ...
        for g_path, p_path in zip(gold_list, pred_list):
            ...
            pred_text = open(p_path, 'r', encoding='utf8').read().strip()
    ...
    def most_frequent(self, k=10):
        ...
        logger.info(
            '           TP before |            TP after |           FP before |            FP after |           FN before |            FN after |')
        logger.info('---------------------+' * 6)
        for i in range(k):
            s = '%14s (%3d) |' * 6
            logger.info(
                s % (*tpb[i], *tpa[i], *fpb[i], *fpa[i], *fnb[i], *fna[i]))
        logger.info('---------------------+' * 6)
        ...
        for i in range(k):
            logger.info(
                '%14s (%3d) |' * 4 % (*tpgb[i], *tpga[i], *tppb[i], *tppa[i]))
        logger.info('---------------------+' * 4)
    ...
    def micro_report(self, gold_dir, pred_dir):
        self.tp, self.tn, self.fp, self.fn = 0, 0, 0, 0
        gold_list = list(sorted(glob.glob(gold_dir)))
        pred_list = list(sorted(glob.glob(pred_dir)))

        print('lista:',gold_list, pred_list)
    ...
    def macro_report(self, gold_dir, pred_dir):
        self.tp, self.tn, self.fp, self.fn = 0, 0, 0, 0
        assert gold_dir[-1] == '*' and pred_dir[-1] == '*'
        gold_dir_without_files = list(sorted(glob.glob(gold_dir[:-1])))
        pred_dir_without_files = list(sorted(glob.glob(pred_dir[:-1])))
        max_fold = len(gold_dir_without_files)
        stats = [0, 0, 0, 0, 0]
        for g_dir, p_dir in zip(gold_dir_without_files, pred_dir_without_files):
            gold_list_folds = list(sorted(glob.glob(g_dir + '*')))
            pred_list_folds = list(sorted(glob.glob(p_dir + '*')))
            golds, preds = self._populate(gold_list_folds, pred_list_folds)
            self.raw_count(np.array(preds), np.array(golds))
    ...
    def average_sentence_length(self):
        ...
        logger.info('Gold average sentence length: %.2f (%.2f)' % (
        avg_sl_golds, std_sl_golds))
        logger.info('Pred average sentence length: %.2f (%.2f)' % (
        avg_sl_preds, std_sl_preds))
        logger.info('Gold nb words per sentence: %.2f' % (
                len(self.golds) / len(indexes_golds)))
        logger.info('Pred nb words per sentence: %.2f' % (
                len(self.preds) / len(indexes_preds)))
    ...
    def correlation(self, method='bns', k=15):
        ...
        ord_res_gold = sorted(res_gold, reverse=True, key=lambda x: x['score'])[
                       :k]

        res_pred = get_bns(self.text_preds)
        ord_res_pred = sorted(res_pred, reverse=True, key=lambda x: x['score'])[
                       :k]
        ...
        for i in range(k):
            logger.info('%32s | %32s |' % (
            ord_res_gold[i]['word'], ord_res_pred[i]['word']))

...
class ErrorAnalysisEditDisfs:
	...
	def errors_by_size(self, golds, preds):
		...
		for size in range(1, 4):
			...
			logger.info('--------------------------------------------------------------------------------+'*2)
			logger.info(' Gold                                                                           | Pred ')
			logger.info('                      REP |                      REV |                      REC |'*2)
			logger.info('--------------------------+'*6)

			_, gw = get_by_size(gold_labels, size)
			_, pw = get_by_size(pred_labels, size)
			...
			if '*' not in self.ignore_mark and len(gw['REP']) > 0:
				gold_rep_w, gold_rep_c = zip(*sorted(gw['REP'].items(), key=lambda x:x[1], reverse=True))
				gold_rep_w = gold_rep_w[:max_k]
				gold_rep_t = sum(gold_rep_c)
				gold_rep_c = list(map(lambda x:x/gold_rep_t, gold_rep_c))[:max_k]

			if '+' not in self.ignore_mark and len(gw['REV']) > 0:
				gold_rev_w, gold_rev_c = zip(*sorted(gw['REV'].items(), key=lambda x:x[1], reverse=True))
				gold_rev_w = gold_rev_w[:max_k]
				gold_rev_t = sum(gold_rev_c)
				gold_rev_c = list(map(lambda x:x/gold_rev_t, gold_rev_c))[:max_k]

			if '$' not in self.ignore_mark and len(gw['REC']) > 0:
				gold_rec_w, gold_rec_c = zip(*sorted(gw['REC'].items(), key=lambda x:x[1], reverse=True))
				gold_rec_w = gold_rec_w[:max_k]
				gold_rec_t = sum(gold_rec_c)
				gold_rec_c = list(map(lambda x:x/gold_rec_t, gold_rec_c))[:max_k]

			if '*' not in self.ignore_mark and len(pw['REP']) > 0:
				pred_rep_w, pred_rep_c = zip(*sorted(pw['REP'].items(), key=lambda x:x[1], reverse=True))
				pred_rep_w = pred_rep_w[:max_k]
				pred_rep_t = sum(pred_rep_c)
				pred_rep_c = list(map(lambda x:x/pred_rep_t, pred_rep_c))[:max_k]

			if '+' not in self.ignore_mark and len(pw['REV']) > 0:
				pred_rev_w, pred_rev_c = zip(*sorted(pw['REV'].items(), key=lambda x:x[1], reverse=True))
				pred_rev_w = pred_rev_w[:max_k]
				pred_rev_t = sum(pred_rev_c)
				pred_rev_c = list(map(lambda x:x/pred_rev_t, pred_rev_c))[:max_k]

			if '$' not in self.ignore_mark and len(pw['REC']) > 0:
				pred_rec_w, pred_rec_c = zip(*sorted(pw['REC'].items(), key=lambda x:x[1], reverse=True))
				pred_rec_w = pred_rec_w[:max_k]
				pred_rec_t = sum(pred_rec_c)
				pred_rec_c = list(map(lambda x:x/pred_rec_t, pred_rec_c))[:max_k]

			for i in range(max_k):
				g1 = '%s (%.2f)' % (gold_rep_w[i], gold_rep_c[i]) if i < len(gold_rep_w) else '%s (%.2f)' % (' ', 0.0)
				g2 = '%s (%.2f)' % (gold_rev_w[i], gold_rev_c[i]) if i < len(gold_rev_w) else '%s (%.2f)' % (' ', 0.0)
				g3 = '%s (%.2f)' % (gold_rec_w[i], gold_rec_c[i]) if i < len(gold_rec_w) else '%s (%.2f)' % (' ', 0.0)
				p1 = '%s (%.2f)' % (pred_rep_w[i], pred_rep_c[i]) if i < len(pred_rep_w) else '%s (%.2f)' % (' ', 0.0)
				p2 = '%s (%.2f)' % (pred_rev_w[i], pred_rev_c[i]) if i < len(pred_rev_w) else '%s (%.2f)' % (' ', 0.0)
				p3 = '%s (%.2f)' % (pred_rec_w[i], pred_rec_c[i]) if i < len(pred_rec_w) else '%s (%.2f)' % (' ', 0.0)
				logger.info(' %24s | %24s | %24s | %24s | %24s | %24s |' % (g1, g2, g3, p1, p2, p3))

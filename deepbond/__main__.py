...
def load_options():
	...
	parser.add_argument('-d', '--dataset', required=True, type=str, help='one of: constituicao/constituicao_mini/pucrs_usp/pucrs_constituicao/controle/ccl/da')
...
def load(features, strategy, tm, model_dir):
...
def segment_sentences(options):
	pass

def remove_disfluences(options):
	pass
...
def run(options):
	...
	strategy = load_strategy(options, dsm)
	lexical_model, lexical_params, prosodic_model, prosodic_params = load_models(options, features, dsm, strategy)
	...
	# k greater than 1: run k-fold cv
	# k equals to 1: run a single fold
	# k equals to 0: just use loaded data to predict test data
	if options.kfold > 1:
...
def cli():
	...
	file_handler = logging.FileHandler('{}.log'.format('data/log/new/'+options.id))

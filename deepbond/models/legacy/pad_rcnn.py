...
from deepbond.models import RecurrentPadded

logger = logging.getLogger(__name__)


class PaddedRCNN(RecurrentPadded):
	...
	def build(self, filter_length=5, rnn='GRU', max_pooling=True):
		...
		cnn1d_text = Convolution1D(nb_filter=nb_filter_text, filter_length=filter_length, border_mode=border_mode, subsample_length=stride, activation='relu')(cnn1d_text_pad)
		...
		cnn1d_audio = Convolution1D(nb_filter=nb_filter_audio, filter_length=filter_length, border_mode=border_mode, subsample_length=stride, activation='relu')(cnn1d_audio_pad)

...
class JustCNN(RecurrentEnsembleDivided):
	...
	def build(self, filter_length=5, rnn='GRU', max_pooling=True):
		...
		# text_timedense = TimeDistributed(Dense(output_dim=self.nb_hidden, activation='sigmoid'))(maxpooling_text)
		text_drop = Dropout(self.dropout_rate)(maxpooling_text)
		...
		# audio_timedense = TimeDistributed(Dense(output_dim=self.nb_hidden, activation='sigmoid'))(maxpooling_audio)
		audio_drop = Dropout(self.dropout_rate)(maxpooling_audio)
...
class JustRNN(RecurrentEnsembleDivided):

	def build(self, rnn='GRU'):
		logger.info('Building...')
		RNN = LSTM if rnn == 'LSTM' else GRU
		...
		f_text = RNN(self.nb_hidden, return_sequences=True, activation='sigmoid')(merge_embedded)	
		b_text = RNN(self.nb_hidden, return_sequences=True, go_backwards=True, activation='sigmoid')(merge_embedded)
		text_model = merge([f_text, b_text], mode='sum', concat_axis=-1)
		...
		self.prosodic_size = self.features.prosodic.nb_phones * self.features.prosodic.nb_features
		sequence_pros = Input(name='input_audio', shape=(self.input_length, self.prosodic_size))
		f_audio = RNN(self.nb_hidden, return_sequences=True, activation='sigmoid')(sequence_pros)
		b_audio = RNN(self.nb_hidden, return_sequences=True, go_backwards=True, activation='sigmoid')(sequence_pros)
		audio_model = merge([f_audio, b_audio], mode='sum', concat_axis=-1)

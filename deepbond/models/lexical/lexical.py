...
class LexicalModel(metaclass=ABCMeta):
    ...
    def prepare_input(self, dataset, strategy):
        ...
        if self.use_embeddings:
            ...
            X.append(X_ind)
        if self.use_pos:
            X_ind_POS = self.features.get_POS(
                dataset.indexes_to_words(X_indexes))
            X_POS = strategy.prepare_input(X_ind_POS)
            X.append(X_POS)
        if self.use_handcrafted:
            X_hc = self.features.get_handcrafted(
                dataset.indexes_to_words(X_indexes), X_ind_POS)
    ...
    def get_batch(self, data_in, data_out, sample_weights, lengths, data_by_length, shuffle=True, kind='train'):
        while True:  # a new epoch
            ...
            for length in lengths:
                ...
                input_data = {}
                if self.use_embeddings:
                    input_data['input_emb'] = np.array(
                        [data_in[len(input_data)][i] for i in indexes])
                if self.use_pos:
                    input_data['input_pos'] = np.array(
                        [data_in[len(input_data)][i] for i in indexes])
                if self.use_handcrafted:
                    input_data['input_hc'] = np.array(
                        [data_in[len(input_data)][i] for i in indexes])
                if len(input_data) == 0:
                    raise Exception('You must use at least one feature.')
                output_data = {'output_source': np.array(
                    [data_out[i] for i in indexes])}
                sw_data = None if sample_weights is None else np.array(
                    [sample_weights[i] for i in indexes])

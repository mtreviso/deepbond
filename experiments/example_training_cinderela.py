...
folds_dir = 'data/transcriptions/folds'

for typ in os.listdir(folds_dir):
  print('Dataset:',typ)
  for fold in os.listdir(os.path.join(folds_dir,typ)):
    print("Fold Number:",fold)
    train_path = os.path.join(folds_dir,typ,fold,'train')
    test_path = os.path.join(folds_dir,typ,fold,'test')
    tagger = Tagger()
    tagger.train(train_path=train_path,test_path=test_path ,dropout=0.2, **args)
